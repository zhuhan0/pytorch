...
from typing import NamedTuple, Callable, Optional, Any, Tuple, Mapping, Type
...
def speedup_experiment_onnx(
    ...
    **kwargs,
    ...
    The main changes are to replace hard coded dynamo logic with ONNX.
    Such as `torch._dynamo.run(model_iter_fn)`.

    NOTE(bowbao): Most of the function is duplicate from `speedup_experiment`, originaly written
    for inductor. In details, the changes are:
        1. Creation of OnnxModel, which handles export, ort initialization.
        2. Creating iobinding with OnnxModel. Essential for CUDA perf measurement.
    ...
    if filled in properly by the driver script.

    TODO(bowbao): Onnx Related code may be refactored to move to under onnx_helper.
...
class OnnxModel:
    ...
    #       The script is not gpu memory efficient. Both pt model and ort inference session are on
    #       gpu. Attempt has been made to move ORT to cuda:1, however ORT perf drop significantly
    #       if it is not on cuda:0.
    ...
    def _init_ort_session(self, model_path: str):
        ...
        # log.debug("Post ort", torch.cuda.memory_summary(device=0, abbreviated=False))
        # log.debug("Post ort", torch.cuda.memory_summary(device=1, abbreviated=False))
        return ort_session
    ...
    def format_pt_outputs(self, pt_outputs):
        ...
        pt_outputs, spec = pytree.tree_flatten(pt_outputs)
    ...
    def create_iobinding(self, pt_inputs, example_outputs):
        ...
        for ort_input, arg in zip(self.onnx_session.get_inputs(), args):
            ...
            device = arg.device
            log.debug(f"ORT binding input {ort_input.name}, size {arg.size()}")
        ...
        for ort_output, output in zip(self.onnx_session.get_outputs(), outputs):
            ...
            log.debug(f"ORT binding output {ort_output.name}, size {output.size()}")
    ...
    def run(self, pt_inputs):
        # NOTE: For CUDA performance testing, must use `run_with_iobinding`.
        # To exclude memory copying overhead for inputs/outputs between cpu and gpu.
...
def optimize_onnx_ctx(
    ...
    # NOTE(bowbao): We are returning our own version of 'run_n_iterations'.
    # The purpose of original 'run_n_iterations' is to provide same api for both training and inference.
    # For ONNX bench, we support only inference for now.
    # In terms of logistics for inference, so far all benchmarks (torchbench.py, huggingface.py, timm_models.py) are
    # calling 'forward' in 'forward_pass', which is assigned to 'model_iter_fn' in 'run_n_iterations'.
    #
    # Hence we need to create our own version of 'run_n_iterations'.
    #   1. Export and cache model.
    #   2. Create iobinding for ORT.
    #   3. Run n iterations.
    #
    # For inductor, this function is used in
    #   1. 'run_performance_test'
    #       * Wrap original model callable to trigger dynamo.
    #       * Check compilation time.
    #       * The separate 'experiment' call will use compiled model automatically.
    #           - For ONNX, we need to export again, since the exported model in `optimize_onnx_ctx` is not
    #             accessible.
    #   2. 'check_accuracy'
    #       * Wrap original model callable to trigger dynamo.
    #       * Run this directly to get compiled model accuracy.
    #
    # Codeflow:
    #   run_n_iterations(model, inputs, n=2) -> model_iter_fn(model, inputs, collect_outputs=True) -> model.forward(inputs)
