...
class MergeSplitCat(PatternEntry):
    def apply(self, match: Match, graph: torch.fx.Graph, node: torch.fx.Node):
        split_nodes = [
            getitem_node.args[0]
            for getitem_node in get_arg_value(node, 0, "tensors")
            if match_node(getitem_node, {operator.getitem})
            and match_node(getitem_node.args[0], {torch.split})
        ]
        if len(set(split_nodes)) == 0:
            return
    ...
    def find_next_users(self, split_node):
        ...
        for getitem_node in split_node.users.keys():
            ...
            for getitem_user in getitem_node.users.keys():
                next_users.add(getitem_user)
        return list(next_users)
    ...
    def get_user_input_list(
        ...
        for user in next_users:
            if match_node(user, {torch.cat, torch.stack}):
    ...
    def get_transform_params(
        ...
        for user_node, user_inputs in zip(next_users, user_inputs_list):
            if not match_node(user_node, {torch.cat, torch.stack}):
    ...
    def replace_cat(
        ...
        for user_node, user_inputs_new, transform_params in zip(
            ...
            for user_input_new, transform_param in zip(
                ...
                if unflatten_params:
                    with graph.inserting_after(user_input_new):
            ...
            with graph.inserting_after(user_inputs_new_transformed[-1]):
    ...
    def erase_old_nodes(self, graph, split_node, next_users):
        ...
        for next_user in next_users:
            if not match_node(next_user, {torch.cat, torch.stack}):
                continue
...
def _split_cat_init():
    from .pre_grad import normalize_split_pass, merge_split_cat_pass
    ...
    for pattern in [
        ...
        pattern = MergeSplitCat(
            pattern=pattern, extra_check=config_flag("split_cat_fx_passes")
        )
        pattern.register(merge_split_cat_pass)
